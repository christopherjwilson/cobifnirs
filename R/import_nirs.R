...
print("imported raw data successfully and converted to tibble.")
},
error = function(e) {
  stop("The nir file: ", nirFile,  " is not in the correct format. Please check the file and try again")
}
)


# Add the start time to the data  for information
...
start_time <- nir1 %>%
  ...
  print("added start time to the data.")
},
warning = function(w) {
  print("Warning: Start time not found in the file. Start time will not be added to the data for ", nirFile)
}
)
...
error = function(e) {
  stop("The nir file: ", nirFile,  " does not contain baseline data. Please check the file and try again")
}
)



## store all of the data from the end of the baseline to the end of the file
tryCatch(
  expr = {
  nirsEndRow <-  nir1 %>% dplyr::filter(str_detect(.$value, '-1	Device Stopped')) %>% dplyr::select(id)
  nirsDataValues <- nir1 %>% dplyr::filter(id > bEndRow$id & id < nirsEndRow$id)## < nirs data have now been stored as: nirsDataValues
  print("nirs data extracted successfully.")
},
error = function(e) {
  stop("The nir file: ", nirFile,  " does not contain expected nirs data. Please check the file and try again")
}
)
...
print("successfully converted nirs data to long format.")
},
error = function(e) {
  stop("The nir file: ", nirFile,  " could not be converted to correct format for analysis. Please check the file and try again")
}
)
